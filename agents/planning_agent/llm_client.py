"""OpenAI-compatible LLM client wrapper for the Planning Agent."""
...
def _build_kwargs(
    ...
    if excluded or uses_max_completion_tokens(config.llm_model):
        logger.debug(
            ...
            f"uses_max_completion_tokens="
            f"{uses_max_completion_tokens(config.llm_model)}"
        )
...
async def chat_completion_stream(
    ...
    Used for final synthesis calls (tool_choice="none").
    """
    cfg = config or AgentConfig()
    ...
    stream = await llm_stream_with_retry(_open_stream, label="planning_stream")
